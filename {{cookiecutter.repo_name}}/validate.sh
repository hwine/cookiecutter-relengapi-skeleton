...
# project name
PROJECT=relengapi-{{cookiecutter.project_name}}
...
if [[ "${PROJECT}" =~ relengapi-[s]keleton ]]; then
    ...
    cd ${tmpbase}/bubbler
    find * -name '*{{cookiecutter.project_name}}*' | while read s; do d=$(echo $s | sed s/{{cookiecutter.project_name}}/bubbler/g); mv $s $d; done
    git grep {{cookiecutter.project_name}} | cut -d: -f 1 | sort -u | while read s; do sed s/{{cookiecutter.project_name}}/bubbler/ < $s > $s~; mv $s~ $s; done
    {
    ...
    } || not_ok "creation of a new blueprint from {{cookiecutter.project_name}} failed"
    cd ${srcdir}
    ...
    cd ${tmpbase}/skeltest
    {
        ...
        ${tmpbase}/skeltest/bin/pip uninstall -y relengapi-{{cookiecutter.project_name}} &&
